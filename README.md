...
マルチモニタ環境では，Office ウィンドウ（スライドショー中は発表者側のスクリーン）があるモニタに合わせて表示・移動してくれます．
...
## PowerPoint：スピーカーノートの読み上げ
スライドショー中，スピーカーノートを自動で読み上げられるようになりました．既定では OFF になっているため，「スライド ショー」タブの ON/OFF・音声設定から有効にしてください．

カイル君自体は SAPI 非対応のため，読み上げには別途 SAPI5 対応のエージェントを選択する必要があります．

リボンの「タグ挿入」から，ノートの選択範囲を音量・速度・ピッチ・無音・強調・スペルアウトなどのタグで挟めます．発表者用のコメント（読み上げない部分）を書いておくことも出来ます．

## PowerPoint：発表時間のお知らせ
PowerPoint でスライドショーを終了すると，カイル君が発表にかかった時間を教えてくれます．

スライドショー中は，スライド番号・発表時間・ラップタイムを吹き出しに常時表示するオーバーレイも表示できます（「スライド ショー」タブのチェックボックスで切り替え）．

## Excel：数式エラーの通知
セルに数式エラー（`#REF!` など）が発生すると，カイル君が気付いて教えてくれます．「数式」タブの ON/OFF トグルから切り替えられます．
...
## キャラクターの追加
`C:\Program Files (x86)\OfficeAgent\agents` フォルダに `.acs` ファイルを追加すると，カイル君以外のキャラクターも選択できるようになります．詳細設定（サイドパネル）の「キャラクター」から切り替えられます．
...
### スピーカーノートの読み上げが動かない場合

カイル君自体は SAPI 非対応のため，読み上げには別途 SAPI5 対応のエージェントを選択する必要があります．詳細設定（サイドパネル）の「キャラクター」から SAPI5 対応のエージェントに切り替えてください．
