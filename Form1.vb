...
Public Class Form1
    ...
    '起動時
    Private Function Form1_Load(sender As Object, e As EventArgs) As Task Handles MyBase.Load
        ...
        Hide()
        Sound.Items.Add("サウンドON")
        Sound.Items.Add("サウンドOFF")
        If My.Settings.DefaultSound = True Then
            Sound.SelectedIndex = 0
        Else
            Sound.SelectedIndex = 1
        End If
        AxAgent.Characters("OfficeAgent").SoundEffectsOn = My.Settings.DefaultSound
        SearchOrGPT.Items.Add("Chat GPT")
        SearchOrGPT.Items.Add("ウェブ検索")
        If My.Settings.DefaultGPT = True Then
            SearchOrGPT.SelectedIndex = 0
        Else
            SearchOrGPT.SelectedIndex = 1
        End If
        api_key_input.Text = My.Settings.API_KEY
        gpt_rule_input.Text = My.Settings.GPT_RULE
        ...
        If SearchOrGPT.SelectedIndex = 0 Then
        ...
        SearchEngine.ToolTipText = "検索エンジンを設定します"
        Sound.ToolTipText = "効果音を設定します"
        SearchOrGPT.ToolTipText = "検索方法を設定します"
        api_key_input.ToolTipText = "OpenAI の API キーを入力します"
        gpt_rule_input.ToolTipText = "Chat GPT の性格を設定します"
    ...
    Private Async Sub Search_Click(sender As Object, e As EventArgs) Handles Search.Click
        With AxAgent.Characters("OfficeAgent")
            If SearchOrGPT.SelectedIndex = 0 Then ' GPTのとき
                ' APIキー
    ...
    'サウンド設定変更時
    Private Sub Sound_Changed(sender As Object, e As EventArgs) Handles Sound.SelectedIndexChanged
        If Sound.SelectedIndex = 0 Then
            My.Settings.DefaultSound = True
            AxAgent.Characters("OfficeAgent").SoundEffectsOn = True
        Else
            My.Settings.DefaultSound = False
            AxAgent.Characters("OfficeAgent").SoundEffectsOn = False
        End If
        My.Settings.Save()
    End Sub

    '検索方法設定変更時
    Private Sub SearchOrGPT_Changed(sender As Object, e As EventArgs) Handles SearchOrGPT.SelectedIndexChanged
        If SearchOrGPT.SelectedIndex = 0 Then
    ...
    'APIキー変更時
    Private Sub APIKEY_Changed(sender As Object, e As EventArgs) Handles api_key_input.TextChanged
        My.Settings.API_KEY = api_key_input.Text
        My.Settings.Save()
    End Sub

    'GPT役作り設定変更時
    Private Sub GPTROLE_Changed(sender As Object, e As EventArgs) Handles gpt_rule_input.TextChanged
        My.Settings.GPT_RULE = gpt_rule_input.Text
        My.Settings.Save()
    End Sub
