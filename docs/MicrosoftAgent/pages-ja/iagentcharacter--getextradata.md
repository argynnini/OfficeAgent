...
canonicalUrl: https://learn.microsoft.com/ja-jp/windows/win32/lwef/iagentcharacter--getextradata
schema: Conceptual
adobe-target: true
...
loc_file_id: Github-175691945.live.MSDN.win32.lwef/iagentcharacter--getextradata.md
...
文字の一部として格納されている追加のデータを取得します。

- 操作が成功したことを示すS\_OKを返します。

- pbszExtraData*を * する
    - 文字の追加データの値を受け取る BSTR のアドレス。 文字の追加データは、Microsoft エージェント文字エディターを使用してコンパイルされるときに定義されます。 文字開発者は、次の文字列を編集して指定できます。文字の ACD ファイル。 この設定は省略可能であり、すべての文字に対して指定することはできません。また、実行時にデータを定義または変更することはできません。 また、提供されるデータの意味は、文字開発者によって定義されます。