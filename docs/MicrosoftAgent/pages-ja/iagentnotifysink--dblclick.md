...
canonicalUrl: https://learn.microsoft.com/ja-jp/windows/win32/lwef/iagentnotifysink--dblclick
schema: Conceptual
adobe-target: true
...
loc_file_id: Github-175691945.live.MSDN.win32.lwef/iagentnotifysink--dblclick.md
...
ユーザーが文字をダブルクリックしたときにクライアント アプリケーションに通知します。
...
このイベントは、文字の入力アクティブ なクライアントに送信されます。 文字のクライアントがいずれも入力アクティブでない場合、サーバーは文字のアクティブなクライアントに通知します。 文字が表示されている場合、サーバーはそのクライアントの入力をアクティブにし、[**IAgentNotifySink::ActivateInputState**](iagentnotifysink--activateinputstate)を送信します。 文字が非表示の場合、文字も自動的に表示されます。