...
canonicalUrl: https://learn.microsoft.com/ja-jp/windows/win32/lwef/iagentcharacter--setposition
schema: Conceptual
adobe-target: true
...
loc_file_id: Github-175691945.live.MSDN.win32.lwef/iagentcharacter--setposition.md
...
- *lLeft*
    - 画面の原点 (左上) を基準にした、文字アニメーション フレームの左端の画面座標 (ピクセル単位)。
- *lTop*
    - 画面の原点 (左上) を基準にした、キャラクター アニメーション フレームの上端の画面座標 (ピクセル単位)。

このプロパティの設定は、文字のすべてのクライアントに適用されます。 不規則な形状の領域ウィンドウに文字が表示される場合でも、文字の位置は四角形のアニメーション フレームに基づいています。

手記

[**MoveTo**](https://www.bing.com/search?q=**MoveTo**) メソッドとは異なり、この関数はキューに登録されません。