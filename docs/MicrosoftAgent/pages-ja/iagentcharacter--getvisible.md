...
canonicalUrl: https://learn.microsoft.com/ja-jp/windows/win32/lwef/iagentcharacter--getvisible
schema: Conceptual
adobe-target: true
...
loc_file_id: Github-175691945.live.MSDN.win32.lwef/iagentcharacter--getvisible.md
...
キャラクターのアニメーション フレームが現在表示されているかどうかを判断します。

- 操作が成功したことを示すS\_OKを返します。

- pbVisible*を * する
    - 文字のフレームが表示されている場合は **True** を受け取り、非表示の場合は False **を** 変数のアドレス。

このメソッドを使用して、文字のフレームが現在表示されているかどうかを確認できます。 文字を表示するには、[**Show**](/ja-jp/windows/desktop/lwef/iagentcharacter--show) メソッドを使用します。 文字を非表示にするには、[**Hide**](/ja-jp/windows/desktop/lwef/iagentcharacter--hide) メソッドを使用します。