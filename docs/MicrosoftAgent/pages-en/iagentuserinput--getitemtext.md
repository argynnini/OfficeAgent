...
canonicalUrl: https://learn.microsoft.com/en-us/windows/win32/lwef/iagentuserinput--getitemtext
...
monikers: []
item_type: Content
source_path: desktop-src/lwef/iagentuserinput--getitemtext.md
cmProducts:
...
Retrieves the voice text for a [**Command**](command-event) alternative passed to the [**IAgentNotifySink::Command**](iagentnotifysink--command) callback.
...
- *pbszText*
    - Address of a BSTR that receives the value of the voice text for the [**Command**](command-event).

If voice input was not the source for the command, for example, if the user selected the command from the character's pop-up menu, the server returns **NULL** for the [**Command**](command-event)'s voice text.